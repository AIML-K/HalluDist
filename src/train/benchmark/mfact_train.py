...
class HallucinationDataset(Dataset):
    def __init__(self, dataset, data_split, weighting=True):
        ...
        if weighting:
            weight = torch.tensor([i['mfact'] for i in dataset[data_split].values()])
...
if __name__ == "__main__":
    for trial in range(5):
        ...
        print(f"=========== Training {dataset} - Trial {trial} ==============")
        model_path = Path(__file__).parents[2] / 'train' / dataset / 'mfact' / f'{trial}trial'
        data = load_json(data_path / f'{dataset}_mfact.json')
        train_dataloader = list(DataLoader(HallucinationDataset(data, "train", weighting=True), batch_size=batch_size, shuffle=True))
        valid_dataloader = list(DataLoader(HallucinationDataset(data, "valid", weighting=True), batch_size=batch_size, shuffle=False))
